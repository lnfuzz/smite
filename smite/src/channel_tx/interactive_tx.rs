...
/// Maximum inputs in the constructed transaction (BOLT 2).
...
/// where 107 is the minimum witness weight. Our wallet inputs are P2WPKH, whose
/// witness is `1` element count `+ 1 + sig + 1 + 33` pubkey; Bitcoin Core
/// grinds for a low-R signature, so `sig` is 71 bytes and the actual weight is
/// the same 107 the floor already charges.
///
/// Charging 108 is a deliberate one-unit overpay, sized for the 72-byte
/// signature Core does not normally produce. It keeps the estimate on the
/// paying side of the requirement: the peer fails the negotiation when our
...
impl SharedTransaction {
    ...
    /// Fee we are responsible for at `feerate_per_kw`, in satoshis, **as the
    /// initiator**.
    ///
    ...
    /// `nVersion` is 2 and `nLockTime` comes from `open_channel2`.
    #[must_use]
