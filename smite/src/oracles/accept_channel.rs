...
fn verify_accepted_open_channel(
    ...
    let announce_channel = open_channel.channel_flags & ANNOUNCE_CHANNEL_FLAG != 0;
...
mod tests {
    ...
    fn open_channel_with_unknown_channel_type_variant() {
        let bits = [
            Features::OPTION_STATIC_REMOTEKEY,
            Features::OPTION_ANCHORS,
            30, // not part of any known channel_type variant
        ];
    ...
    fn opener_cannot_cover_anchor_outputs() {
        ...
        oc.tlvs.channel_type = Some(ChannelTypeVariant::Anchors.encode());
    ...
    fn accept_channel_channel_type_mismatch_with_open_channel() {
        let mut ac = accept_channel();
        ac.tlvs.channel_type = Some(ChannelTypeVariant::Anchors.encode());
    ...
    // NOTE: Validation is skipped, but once we add support for 0FC, this should
    // still be accepted since the `shared_anchor` amount is 0.
    #[test]
    ...
    // commitment fee and anchors, this should be rejected.
    #[test]
