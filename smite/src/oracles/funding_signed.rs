...
impl Oracle<FundingSignedContext<'_>> for FundingSignedOracle {
    fn evaluate(&self, context: &FundingSignedContext<'_>) -> Result<(), Violation> {
        ...
        // Check that this is the first `funding_signed` for this channel.
        //
        // NOTE: Besides a genuine resend, this also catches a valid
        // `funding_signed` for a new channel whose channel_id collides with an
        // existing one due to the XOR relationship between channel_id and the
        // funding outpoint. BOLT 2 does not specify how to handle this, but
        // LND, LDK, and Eclair all reject such channels. Since channel_id is
        // reused across messages, accepting a collision could cause
        // cross-channel state contamination and lead to more serious bugs. Such
        // collisions are also extremely unlikely to occur naturally, so the
        // target should reject them.
        // see: https://github.com/ElementsProject/lightning/issues/9274#issuecomment-5316110622
        if channel.funding_signed_received {
            return Err(Violation::InvalidFundingSigned(
                context.funding_signed.channel_id,
                "duplicate funding_signed: channel already funded".to_string(),
            ));
        }
        ...
        //
        // NOTE: If the colliding channel never received its own
...
mod tests {
    use super::*;
    use crate::bolt::{COMPACT_SIGNATURE_SIZE, ChannelId, Features};
    ...
    #[test]
    fn duplicate_funding_signed() {
        let mut channel = channel_state();
        channel.funding_signed_received = true;

        assert_fail(
            &funding_signed(&channel),
            Some(&channel),
            "duplicate funding_signed: channel already funded",
        );
    }
