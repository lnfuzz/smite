...
impl Features {
    ...
    pub fn set_bit(&mut self, bit: FeatureBit) {
        ...
        let required_len = byte_offset + 1;
        if self.0.len() < required_len {
            // Pad with leading zeroes to the required length.
            self.0.splice(0..0, vec![0u8; required_len - self.0.len()]);
        }
...
impl PartialEq for Features {
    ...
    fn eq(&self, other: &Self) -> bool {
        // Leading zero bytes are padding, so they carry no feature bits.
...
mod tests {
    ...
    fn supports_feature_uses_big_endian_bit_order() {
        ...
        assert!(Features::from(vec![0x40, 0x00, 0x00]).supports_feature(22));
        assert!(Features::from(vec![0x80, 0x00, 0x00]).supports_feature(22));
        // No support.
        assert!(!Features::from(vec![0x00, 0x00, 0x40]).supports_feature(22));
        assert!(!Features::from(vec![0x00, 0x00, 0x80]).supports_feature(22));
        assert!(!Features::from(vec![]).supports_feature(22));
        assert!(!Features::from(vec![0xff, 0xff]).supports_feature(22));
        assert!(!Features::from(vec![0x00, 0x10]).supports_feature(22));
    }
    ...
    fn equality_with_same_and_different_bits() {
        ...
        // Same bytes result in the same features.
        assert_eq!(
