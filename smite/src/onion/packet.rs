...
impl OnionPacket {
    ...
    pub fn encode(&self) -> Vec<u8> {
        ...
        self.public_key.write(&mut out);
        self.hop_payloads.write(&mut out);
        self.hmac.write(&mut out);
        out
    }
    ...
    pub fn decode(data: &[u8]) -> Result<Self, OnionError> {
        ...
        let public_key = PublicKey::read(&mut cursor)?;
        let hop_payloads: [u8; PAYMENT_HOP_PAYLOADS_SIZE] = WireFormat::read(&mut cursor)?;
        let hmac: [u8; HMAC_SIZE] = WireFormat::read(&mut cursor)?;
        Ok(Self {
            version,
            public_key,
            hop_payloads,
