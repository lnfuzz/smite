...
use smite::bolt::{
    ChannelId, ChannelTypeVariant, Error as ErrorMessage, FeatureBit, Features, Init, InitTlvs,
    Message, OpenChannel, OpenChannelTlvs,
};
...
/// sequential and each is retired with an `error`.
...
impl<T: Target> SnapshotSetup<T> for PostInitSetup {
    fn setup(target: &T) -> Result<(NoiseConnection, ProgramContext), ScenarioError> {
        let (conn, our_init) = establish_connection(target)?;
