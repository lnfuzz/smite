...
Replays a single input against a campaign's target and scenario, logging output to the terminal.
...
If the image's digest no longer matches the one recorded for the campaign (e.g. it was rebuilt under the same tag), a warning is logged.
